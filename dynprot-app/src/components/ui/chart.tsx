...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  const generateCSSVariables = (theme: string) => {
    const styles: Record<string, string> = {};
    
    colorConfig.forEach(([key, itemConfig]) => {
      const color = itemConfig.theme?.[theme as keyof typeof itemConfig.theme] || itemConfig.color;
      if (color && typeof color === 'string' && /^#[0-9A-Fa-f]{3,8}$|^rgb|^hsl|^[a-zA-Z]+$/.test(color)) {
  ...
  // Create style elements for each theme
  return (
    <>
      {Object.entries(THEMES).map(([theme, prefix]) => {
        const styles = generateCSSVariables(theme);
